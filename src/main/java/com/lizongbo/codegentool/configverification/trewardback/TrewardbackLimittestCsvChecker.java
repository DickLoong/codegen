...
public class TrewardbackLimittestCsvChecker implements CsvChecker {
	...
	public List<String> checkCsv(String csvPath) {
		...
		for (String[] item : colList) {
			...
			int giftPackId = Integer.parseInt(CSVUtil.getColValue("fifth_diamonds_reward", item, colList));
			if(new TbackpackGiftpackCsvChecker().getCache(giftPackId) == null && giftPackId != 0){
				errorList.add("giftpack not exists|" + giftPackId);
			}
			giftPackId = Integer.parseInt(CSVUtil.getColValue("fifth_gold_reward", item, colList));
			if(new TbackpackGiftpackCsvChecker().getCache(giftPackId) == null && giftPackId != 0){
				errorList.add("giftpack not exists|" + giftPackId);
			}
		}
