 ...
 *
 */
public class ScpClientPackageToReleaseVersionMajiabao {
	...
	public static void main(String[] args) {
		...
		//创建目录
		DeployCommandUtil.CreateClientReleaseTagRoot(majiabaoName, clientDeployProp, tag);
		
		//将AB压缩后放到版本服务器(scpClientTagToReleaseVersion.pl)
		if (new File(abPath).exists()){
			String abZipFile = "/tmp/" + majiabaoName + "/AB_" + platform + ".zip";
			ServerContainerGenTool.zipDir(abPath, abZipFile, ".");
			ScpCommandUtil.ScpClientFileToReleaseVersionServer(majiabaoName, clientDeployProp, abZipFile, tag);
		}
		else{
		...
		ScpCommandUtil.ScpClientFileToReleaseVersionServer(majiabaoName, clientDeployProp, pbBytesPath, tag);
	}
